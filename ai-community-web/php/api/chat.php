...
// 意圖對應 service_type 的映射
$intentServiceTypeMap = [
    'restaurant_booking' => '01',
    'shopping'           => '02',
    'cleaning'           => '03',
    'repair'             => '04',
    'elderly_care'       => '05',
...
if ($aiResult['success']) {
    ...
    if ($parsed && isset($parsed['reply'])) {
        ...
        // 如果辨識到意圖，查詢對應廠商名單
        if ($intent !== 'none' && isset($intentServiceTypeMap[$intent])) {
            $serviceType = $intentServiceTypeMap[$intent];
            $vendors = dbFetchAll(
    ...
    foreach ($intents as $intent) {
        foreach ($intent['keywords'] as $keyword) {
            if (mb_strpos($message, $keyword) !== false) {
                ...
                // 查詢對應廠商
                if (isset($intentServiceTypeMap[$intent['intent']])) {
                    $serviceType = $intentServiceTypeMap[$intent['intent']];
                    $vendors = dbFetchAll(
                        "SELECT v.vendor_id, v.vendor_name, v.rating_avg, v.contact_phone
                         FROM pms_vendor v
                         INNER JOIN pms_vendor_service_type vst ON v.vendor_id = vst.vendor_id
